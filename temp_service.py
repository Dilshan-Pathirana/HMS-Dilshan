...
class DoctorScheduleService:
    ...
    async def list_modifications(
        session: AsyncSession, schedule_id: Optional[str] = None, doctor_id: Optional[str] = None
    ) -> List[ScheduleModification]:
        q = select(ScheduleModification)
        if schedule_id:
            q = q.where(ScheduleModification.schedule_id == schedule_id)
        if doctor_id:
            q = q.where(ScheduleModification.doctor_id == doctor_id)
    ...
    async def update_modification(
        session: AsyncSession, mod_id: str, data: dict, modified_by: str
    ) -> ScheduleModification:
        ...
        # If status is changing to approved/rejected, set approved_by
        if "status" in data and data["status"] in ["approved", "rejected"]:
            mod.approved_by = modified_by
