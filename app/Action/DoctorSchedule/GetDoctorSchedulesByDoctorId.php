...
class GetDoctorSchedulesByDoctorId
{
    public function __invoke($doctor_id): array
    {
        try {
            $doctorSchedules = DB::table('doctor_schedules')
                ...
                ->where('doctor_schedules.doctor_id', $doctor_id)
                ->get();
            ...
            return CommonResponse::sendSuccessResponseWithData('doctorSchedule', $doctorSchedules);
        } catch (\Exception $e) {
            Log::error('GetDoctorSchedulesByDoctorIdError: '.$e->getMessage());
