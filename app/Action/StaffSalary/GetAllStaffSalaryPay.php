...
class GetAllStaffSalaryPay
{
    public function __invoke(): array
    {
        try {
            $staffSalary = DB::table('staff_salary_pay')
                ...
                ->select(
                    ...
                    DB::raw('SUM(employee_ot.hours_worked) as total_hours_worked'),
                    DB::raw('SUM(employee_ot.total_ot_amount) as total_ot_amount')
                )
                ->groupBy(
                    'staff_salary_pay.id',
                    'staff_salary_pay.user_id',
            ...
            return CommonResponse::sendSuccessResponseWithData('staffSalaryPay', $staffSalary);
        } catch (\Exception $e) {
            Log::error('GetAllStaffSalaryPayError: '.$e->getMessage());
