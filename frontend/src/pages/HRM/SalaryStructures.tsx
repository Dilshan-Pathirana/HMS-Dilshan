...
const SalaryStructures: React.FC = () => {
    ...
    const fetchStructures = async () => {
        try {
            ...
            const response = await api.get('/hrm/super-admin/salary-structures', {
                headers: { Authorization: `Bearer ${token}` },
                params
            });
            if (response.data.status === 200) {
    ...
    const fetchStats = async () => {
        try {
            ...
            const response = await api.get('/hrm/super-admin/salary-structures/stats', {
                headers: { Authorization: `Bearer ${token}` },
                params
            });
            if (response.data.status === 200) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = isEditMode
                ? `/hrm/super-admin/salary-structures/${selectedStructure?.id}`
                : '/hrm/super-admin/salary-structures';
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await api.delete(`/hrm/super-admin/salary-structures/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchStructures();
            fetchStats();
    ...
    const handleCopyToBranch = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await api.post('/hrm/super-admin/salary-structures/copy-to-branch', {
