...
const PostDetailPage: React.FC = () => {
    ...
    const fetchPost = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/medical-insights/posts/${slug}`);
...
*Note: Individual results may vary. Always consult with a qualified practitioner before starting any treatment.*
    ...
    const handleLike = async () => {
        ...
        try {
            await api.post(`/medical-insights/posts/${post?.id}/like`);
            setIsLiked(!isLiked);
            if (post) {
                setPost({
                    ...post,
    ...
    const handleRate = async (rating: number) => {
        ...
        try {
            await api.post(`/medical-insights/posts/${post?.id}/rate`, { rating });
    ...
    const handleSubmitComment = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.post(`/medical-insights/posts/${post?.id}/comments`, {
                content: newComment
            });
            ...
            const demoComment: Comment = {
                ...
                status: 'visible',
                created_at: new Date().toISOString(),
                user: { id: Number(userId) || 0, name: 'You' },
                replies: []
            };
    ...
    const handleSubmitQuestion = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.post(`/medical-insights/posts/${post?.id}/questions`, {
            ...
            const demoQuestion: Question = {
                ...
                created_at: new Date().toISOString(),
                answered_at: null,
                patient: { id: Number(userId) || 0, name: 'You' }
            };
