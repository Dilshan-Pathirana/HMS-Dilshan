...
const BranchAdminCashierManagement = () => {
    ...
    const loadCashiers = async () => {
        ...
        setError("");
        try {
            const response = await api.get(`/branch-admin/pos/cashiers?date=${selectedDate}`, {
    ...
    const loadEODStatus = async () => {
        ...
        try {
            const response = await api.get(`/branch-admin/pos/cashiers/eod-status?date=${selectedDate}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
            });
