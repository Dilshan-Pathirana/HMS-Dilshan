...
export const SuperAdminFeedbacks: React.FC = () => {
    ...
    // Fetch feedbacks when branch or filters change
    useEffect(() => {
        fetchFeedbacks();
        fetchStats();
    }, [selectedBranchId, filterCategory, filterStatus, filterPriority, filterUserType, showFlagged]);

    const fetchBranches = async () => {
        try {
            setLoadingBranches(true);
            const response = await api.get('/get-branches');
            if (response.data.status === 200 && response.data.branches) {
    ...
    const fetchFeedbacks = async () => {
        try {
            ...
            if (selectedBranchId !== 'all') params.branch_id = selectedBranchId;
            ...
            const response = await api.get('/branch-admin/feedbacks', { params });
            ...
            setFeedbacks([
                ...
                },
                {
                    ...
                    branch_id: 2,
                    branch_name: 'North Branch - Jaffna',
                    doctor_id: null,
                    doctor_name: null,
                    category: 'facility',
    ...
    const fetchStats = async () => {
        try {
            ...
            const response = await api.get('/branch-admin/feedbacks/stats', { params });
            if (response.data.status === 200) {
    ...
    const handleRespond = async () => {
        ...
        setSubmitting(true);
        try {
            await api.post(`/branch-admin/feedbacks/${selectedFeedback.id}/respond`, {
    ...
    const handleUpdateStatus = async (id: number, status: string) => {
        try {
            await api.post(`/branch-admin/feedbacks/${id}/status`, { status });
            fetchFeedbacks();
            fetchStats();
        } catch (error) {
    ...
    const handleToggleFlag = async (id: number) => {
        try {
            await api.post(`/branch-admin/feedbacks/${id}/flag`);
            fetchFeedbacks();
            fetchStats();
        } catch (error) {
    ...
    const handleUpdatePriority = async (id: number, priority: string) => {
        try {
            await api.post(`/branch-admin/feedbacks/${id}/priority`, { priority });
            fetchFeedbacks();
        } catch (error) {
    ...
    const getSelectedBranchName = () => {
        ...
        return branch?.center_name || 'Unknown Branch';
    };
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-neutral-600 mt-1">View and respond to feedback from all branches</p>
                    </div>
                    <button
                ...
                {/* Branch Selector */}
                <div className="bg-gradient-to-r from-primary-500 to-indigo-600 rounded-xl p-6 shadow-lg">
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-white/80 text-sm">Select Branch</p>
                                <p className="text-white font-semibold text-lg">{getSelectedBranchName()}</p>
                            </div>
                        </div>
                        <div className="flex-1 md:max-w-md">
                            <select
                                ...
                                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:ring-2 focus:ring-white/30 focus:border-white/30 backdrop-blur-sm"
                                style={{ color: 'white' }}
                            >
                                <option value="all" style={{ color: '#1f2937' }}>All Branches</option>
                                {branches.map((branch) => (
                                    <option key={branch.id} value={branch.id} style={{ color: '#1f2937' }}>
                                        {branch.center_name}
                                    </option>
                                ))}
                            </select>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            {selectedBranchId !== 'all' && (
                                ...
                                >
                                    View feedbacks from all branches
                                </button>
                            )}
                        </div>
                    ) : (
                        ...
                        <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl">
                            <div className="p-6 border-b border-gray-100">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-neutral-500 mt-1">Responding on behalf of branch admin</p>
                                    </div>
                                    <button
                            ...
                            <div className="p-6">
                                {selectedFeedback.branch_name && (
                                    <div className="mb-4 p-2 bg-blue-50 rounded-lg flex items-center gap-2 text-blue-700 text-sm">
