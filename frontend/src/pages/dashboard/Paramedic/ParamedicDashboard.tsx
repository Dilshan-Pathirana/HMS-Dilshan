...
export const ParamedicDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get('/paramedic/dashboard-stats');
        ...
        <DashboardLayout userName={userName} userRole="Paramedic" profileImage={profileImage} sidebarContent={<SidebarMenu items={ParamedicMenuItems} />}>
            <div className="space-y-6">
                <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                    ...
                    <p className="text-neutral-600 mt-1">Emergency response and ambulance services management.</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                        <h2 className="text-lg font-semibold text-neutral-800 mb-4">Active Calls</h2>
                        <div className="space-y-3">
                            {stats.activeEmergencies > 0 ? (
                                <div className="flex items-center gap-3 p-4 bg-error-50 rounded-lg border border-red-200">
                                    <AlertCircle className="w-6 h-6 text-error-600" />
                                    <div className="flex-1">
                                        <p className="font-medium text-neutral-800">Emergency - Cardiac Arrest</p>
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-neutral-500">No active emergencies</p>
                                </div>
                            )}
                        </div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                        ...
                        <div className="space-y-3">
                            <button className="w-full flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-red-50 to-red-100 rounded-lg hover:from-red-100 hover:to-red-200 transition-all text-left border border-red-200">
                                <Ambulance className="w-5 h-5 text-error-600" />
                                <div><p className="font-medium text-neutral-800">Respond to Emergency</p><p className="text-xs text-neutral-500">New emergency call</p></div>
                            </button>
                            <button className="w-full flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-lg hover:from-emerald-100 hover:to-blue-100 transition-all text-left">
                                ...
                                <div><p className="font-medium text-neutral-800">Equipment Check</p><p className="text-xs text-neutral-500">Verify equipment status</p></div>
                            </button>
                            <button className="w-full flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-lg hover:from-emerald-100 hover:to-blue-100 transition-all text-left">
                                <Clock className="w-5 h-5 text-purple-600" />
                                <div><p className="font-medium text-neutral-800">Log Response</p><p className="text-xs text-neutral-500">Record emergency response</p></div>
                            </button>
                        </div>
                    </div>
                </div>
