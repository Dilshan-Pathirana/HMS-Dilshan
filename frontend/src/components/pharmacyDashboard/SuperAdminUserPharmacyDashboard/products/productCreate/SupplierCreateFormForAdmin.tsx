...
const SupplierCreateFormForAdmin: React.FC<SupplierCreateFormProps> = ({
    setIsAddingSupplier,
    fetchSuppliers,
}) => {
    ...
    const handleSupplierInsert = async (
        ...
        try {
            ...
            if (response.data.status === 200 || response.status === 200) {
                ...
                console.error("Failed to add supplier:", response.data?.message || response.statusText);
                alert.error(response.data?.message || "Failed to add supplier");
            }
        } catch (error) {
            if (axios.isAxiosError(error)) {
                const errorMsg = error.response?.data?.message || error.response?.data || error.message;
                console.error("Error adding supplier:", errorMsg);
                alert.warn(typeof errorMsg === 'string' ? errorMsg : "Error adding supplier");
            } else {
...
export default SupplierCreateFormForAdmin;
