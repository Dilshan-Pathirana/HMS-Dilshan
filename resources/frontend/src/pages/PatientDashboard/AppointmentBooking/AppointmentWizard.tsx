...
const AppointmentWizard: React.FC = () => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      const [citiesRes, specializationsRes] = await Promise.all([
        axios.get('/api/appointments/cities'),
        axios.get('/api/appointments/specializations'),
      ]);
  ...
  const searchDoctors = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/appointments/doctors/search?${params.toString()}`);
  ...
  const fetchPayHereData = async (booking: BookingResult) => {
    try {
      const response = await axios.post(
        `/api/patient/appointments/${booking.id}/prepare-payment`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
