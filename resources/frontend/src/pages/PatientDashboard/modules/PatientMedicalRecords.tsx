...
const RecordsOverview: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRecords = async () => {
            try {
                // Fetch visit history
                const visitsRes = await axios.get(`/api/patient/visits/${userId}`);
            ...
            try {
                // Fetch lab reports
                const labsRes = await axios.get(`/api/patient/lab-reports/${userId}`);
...
const VisitHistory: React.FC = () => {
    ...
    useEffect(() => {
        const fetchVisits = async () => {
            try {
                const response = await axios.get(`/api/patient/visits/${userId}`);
...
const LabReports: React.FC = () => {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                const response = await axios.get(`/api/patient/lab-reports/${userId}`);
                if (response.data.status === 200) {
