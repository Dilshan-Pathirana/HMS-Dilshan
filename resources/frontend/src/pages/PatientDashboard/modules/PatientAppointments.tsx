...
const AppointmentsList: React.FC = () => {
    ...
    const fetchAppointments = async () => {
        try {
            const response = await axios.get(`/api/get-patient-appointments/${userId}`);
    ...
    const cancelAppointment = async (appointmentId: string) => {
        ...
        try {
            ...
            await axios.post(`/api/patient/appointments/${appointmentId}/cancel`, {
                reason: cancelReason,
                confirmed: true
            });
...
export const BookAppointment: React.FC = () => {
    ...
    const fetchDoctorSchedules = async (branchId: string) => {
        setLoading(true);
        try {
            const response = await axios.get('/api/get-doctor-schedules', {
    ...
    const handleProceedToPayment = async () => {
        ...
        try {
            ...
            const response = await axios.post('/api/create-patient-appointment', payload);
...
const RescheduleAppointment: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEligibility = async () => {
            ...
            try {
                ...
                const response = await axios.get(`/api/patient/appointments/${id}/reschedule-eligibility`, {
                ...
                if (response.data.status === 200) {
                    ...
                    const detailsResponse = await axios.get(`/api/patient/appointments/${id}`, {
                        headers: { Authorization: `Bearer ${token}` }
                    });
                    ...
                    try {
                        const schedulesResponse = await axios.get(`/api/appointments/doctors/${doctorId}/schedules`);
    ...
    const loadSlots = async (date: string) => {
        ...
        try {
            ...
            const response = await axios.post('/api/appointments/doctors/slots-with-times', {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            const response = await axios.post(`/api/patient/appointments/${id}/reschedule`, {
                new_date: newDate,
