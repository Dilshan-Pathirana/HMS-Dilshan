...
const EmployeeScheduleAcknowledgment: React.FC = () => {
    ...
    const fetchEmployeeSchedules = async () => {
        try {
            ...
            try {
                const response = await axios.get('/api/hrm/employee/shifts');
            ...
            const legacyResponse = await axios.get('/api/get-all-shifts-user');
    ...
    const fetchBranchInfo = async (branchIds: string[]) => {
        try {
            for (const branchId of branchIds) {
                const response = await axios.get(`/api/branch/${branchId}`);
                if (response.data.status === 200) {
    ...
    const handleAcknowledge = async (scheduleId: string) => {
        try {
            ...
            const response = await axios.put(
                `/api/hrm/employee/shifts/${scheduleId}/acknowledge`,
                {}
            );
