...
const DoctorPatientQueue: React.FC = () => {
    ...
    const fetchQueueData = async (showRefresh = false) => {
        try {
            ...
            const response = await axios.get(`/api/get-doctor-all-schedule/${userId}`);
            if (response.data.status === 200) {
                ...
                if (todaySchedule) {
                    const appointmentsResponse = await axios.get(
                        `/api/get-doctor-schedule-appointments/${userId}/${todaySchedule.branch_id}/${todaySchedule.id}`
                    );
    ...
    const updatePatientStatus = async (patientId: string, newStatus: string) => {
        try {
            await axios.put(`/api/update-appointment-status/${patientId}`, {
                status: newStatus
            });
            fetchQueueData();
        } catch (error) {
