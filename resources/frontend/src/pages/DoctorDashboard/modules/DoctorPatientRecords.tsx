...
const DoctorPatientRecords: React.FC = () => {
    ...
    const searchPatients = async () => {
        ...
        try {
            ...
            // const response = await axios.get(`/api/search-patients?q=${searchTerm}`);
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-500">Search and view patient medical history</p>
            </div>
                        ...
                        <div className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
                            {patients.length === 0 ? (
                                <div className="p-8 text-center">
                                    ...
                                    <p className="text-gray-500 text-sm">Search for patients to view their records</p>
                                </div>
                            ) : (
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-gray-500">{selectedPatient.phone} • {selectedPatient.email}</p>
                                ...
                                <div className="p-6">
                                        ...
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="p-4 bg-gray-50 rounded-lg">
                                                    ...
                                                    <p className="text-2xl font-bold text-gray-800">{selectedPatient.visits.length}</p>
                                                </div>
                                                <div className="p-4 bg-gray-50 rounded-lg">
                                                    <p className="text-sm text-gray-500">Last Visit</p>
                                                    <p className="text-lg font-semibold text-gray-800">{selectedPatient.last_visit}</p>
                                                </div>
                                            </div>
                                            
                                            <div>
                                                <h4 className="font-medium text-gray-800 mb-3">Recent Diagnoses</h4>
                                                {selectedPatient.visits.slice(0, 3).map((visit) => (
                                                    <div key={visit.id} className="p-3 border border-gray-200 rounded-lg mb-2">
                                                        <div className="flex justify-between">
                                                            <p className="font-medium text-gray-700">{visit.diagnosis}</p>
                                                            <p className="text-sm text-gray-500">{visit.date}</p>
                                                        </div>
                                                        <p className="text-sm text-gray-500 mt-1">Dr. {visit.doctor_name}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                                ...
                                                selectedPatient.visits.map((visit) => (
                                                    <div key={visit.id} className="p-4 border border-gray-200 rounded-lg">
                                                        <div className="flex justify-between mb-2">
                                                            ...
                                                            <span className="text-sm text-gray-500">{visit.branch_name}</span>
                                                        </div>
                                                        <p className="text-gray-700 mb-2">{visit.diagnosis}</p>
                                                        <p className="text-sm text-gray-500">Consulted with Dr. {visit.doctor_name}</p>
                                                        {visit.notes && (
                                                            <p className="text-sm text-gray-600 mt-2 p-2 bg-gray-50 rounded">{visit.notes}</p>
                                                        )}
                                                    </div>
                                                ))
                                            )}
                                    ...
                                    {activeTab === 'investigations' && (
                                        <div className="text-center py-8">
                                            ...
                                            <p className="text-gray-500">Investigation history will be displayed here</p>
                                        </div>
                                    )}
                                </div>
                            </div>
