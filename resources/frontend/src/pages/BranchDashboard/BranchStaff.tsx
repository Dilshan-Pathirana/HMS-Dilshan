...
const BranchStaff: React.FC = () => {
  ...
  const fetchStaffAssignments = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://127.0.0.1:8000/api/v1/branch-staff?branch_id=${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post('http://127.0.0.1:8000/api/v1/branch-staff', payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleRemove = async (assignmentId: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://127.0.0.1:8000/api/v1/branch-staff/${assignmentId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchStaffAssignments();
    } catch (err: any) {
