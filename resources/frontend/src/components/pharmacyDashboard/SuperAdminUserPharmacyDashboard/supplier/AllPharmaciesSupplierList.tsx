...
const AllPharmaciesSupplierList: React.FC = () => {
    ...
    const fetchSuppliers = async (page: number = 1) => {
        try {
            ...
            if (response.data.status === 200) {
                ...
                setCountries([...new Set(allSuppliers.map((s: Supplier) => s.supplier_country).filter(Boolean))]);
                setCities([...new Set(allSuppliers.map((s: Supplier) => s.supplier_city).filter(Boolean))]);
                setTypes([...new Set(allSuppliers.map((s: Supplier) => s.supplier_type).filter(Boolean))]);
                const allProducts = allSuppliers
                    .map((s: Supplier) => s.products_supplied)
                    .filter(Boolean)
                    .flatMap((p: string) => p.split(',').map(item => item.trim()));
                setProductTypes([...new Set(allProducts)]);
            }
        } catch (err: any) {
    ...
    const saveEditedSupplier = async (updatedSupplier: Supplier) => {
        try {
            const token = localStorage.getItem('token');
