...
const SupplierDashboard: React.FC = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const response = await axios.get('http://127.0.0.1:8000/api/supplier/dashboard', {
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Welcome, {supplier?.supplier_name}</p>
                        </div>
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p className="text-gray-800 font-medium">{supplier?.contact_person || 'N/A'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Email</p>
                            <p className="text-gray-800 font-medium">{supplier?.contact_email || 'N/A'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Phone</p>
                            <p className="text-gray-800 font-medium">{supplier?.contact_number || 'N/A'}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Supplier Type</p>
                            <p className="text-gray-800 font-medium">{supplier?.supplier_type || 'N/A'}</p>
                        </div>
                        <div className="md:col-span-2">
                            <p className="text-sm text-gray-500">Products Supplied</p>
                            <p className="text-gray-800 font-medium">{supplier?.products_supplied || 'N/A'}</p>
                        </div>
                        {supplier?.pharmacy_name && (
                            <div className="md:col-span-2">
                                <p className="text-sm text-gray-500">Pharmacy</p>
                                <p className="text-gray-800 font-medium">{supplier.pharmacy_name}</p>
                            </div>
                        )}
                    </div>
                </div>
